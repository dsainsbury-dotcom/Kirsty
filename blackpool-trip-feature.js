(()=>{
  ...
  function addBlackpoolTrip(){
    ...
    if(home && !document.getElementById('blackpool-trip-card')){
      const card=document.createElement('div');
      card.className='card';
      card.id='blackpool-trip-card';
      card.style.marginBottom='16px';
      card.innerHTML=`<span class="pill">LOOKING FORWARD TO</span><h3 style="margin:10px 0 6px">Blackpool with Mum & Dad</h3><p style="margin:0 0 14px">3-6 September 2026. A proper little family adventure, planned to make the trip feel special before we even leave.</p><a href="${poster}" target="_blank" rel="noopener" aria-label="Open Blackpool trip poster"><img src="${poster}" alt="Blackpool trip plan, 3rd to 6th September 2026" style="display:block;width:min(100%,520px);height:auto;margin:0 auto;border-radius:14px;border:1px solid var(--line);box-shadow:0 12px 32px rgba(0,0,0,.24)"></a><small style="display:block;margin-top:12px">Family · connection · enjoyment · living again</small>`;
  ...
  let tries=0;
  const timer=setInterval(()=>{addBlackpoolTrip();tries++;if((document.getElementById('blackpool-trip-card')&&document.getElementById('journey-blackpool-plan')&&document.getElementById('blog-blackpool-plan')&&document.getElementById('library-blackpool-poster'))||tries>40)clearInterval(timer);},200);
})();
